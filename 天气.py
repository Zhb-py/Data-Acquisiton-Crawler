import pandas as pd
import requests
import time
import base64
from lxml import etree
from openpyxl import Workbook

url = "http://127.0.0.1:5000/city_weather?city="
city = ['青岛', '开封', '苏州', '扬州', '烟台', '丽江', '桂林', '三亚', '厦门', '大理']
headers = {
    'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36',
    'Referer': 'http://127.0.0.1:5000/',
    'Cookie': 'salt="\302\210D\303\2609\302\221\007\302\230\302\211f9\303\254J:U\027\303\205V\302\276\302\213\303\257\303\227\303\230\303\223\303\246\302\230*4"'
}

for c in city:
    wb = Workbook()
    ws = wb.active
    headers1 = ['城市', '日期', '最高气温', '最低气温', '天气', '风向']
    ws.append(headers1)
    weather_list = []

    for page in range(1, 13):
        try:
            del str
        except NameError:
            pass
        t = str(int(time.time() * 1000))
        t64 = base64.b64encode(t.encode('utf8')).decode('utf8')
        url2 = url + c + "&page=" + str(page) + "&TOKEN=" + t64
        print(url2)
        res = requests.get(url=url2, headers=headers)
        time.sleep(2)
        html_ds = res.content
        html_ds = etree.HTML(html_ds)
        weather_f = html_ds.xpath('//tr')

        for f in weather_f[1:]:
            weather_l = []
            str = f.xpath('.//td/text()')
            weather_l.append(c)
            weather_l.append(str[0])
            weather_l.append(str[1])
            weather_l.append(str[2])
            weather_l.append(str[3])
            weather_l.append(str[4])
            weather_list.append(weather_l)
            ws.append(weather_l)

    wb.save(c + "_day.csv")
    df = pd.DataFrame(weather_list)
    df.columns = headers1
    print(df)
